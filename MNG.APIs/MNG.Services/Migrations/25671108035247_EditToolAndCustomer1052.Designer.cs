// <auto-generated />
using System;
using MNG.Services.Data;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

namespace MNG.Services.Migrations
{
    [DbContext(typeof(AppDb))]
    [Migration("25671108035247_EditToolAndCustomer1052")]
    partial class EditToolAndCustomer1052
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "2.1.14-servicing-32113")
                .HasAnnotation("Relational:MaxIdentifierLength", 128)
                .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

            modelBuilder.Entity("MNG.Models.ChemicalCompositionInFurnace", b =>
                {
                    b.Property<string>("Code")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20);

                    b.Property<float?>("AlMax");

                    b.Property<float?>("AlMin");

                    b.Property<float?>("CCEMax");

                    b.Property<float?>("CCEMin");

                    b.Property<float?>("CMax");

                    b.Property<float?>("CMin");

                    b.Property<float?>("CrMax");

                    b.Property<float?>("CrMin");

                    b.Property<float?>("CuMax");

                    b.Property<float?>("CuMin");

                    b.Property<string>("Description");

                    b.Property<float?>("MgMax");

                    b.Property<float?>("MgMin");

                    b.Property<float?>("MnMax");

                    b.Property<float?>("MnMin");

                    b.Property<float?>("MoMax");

                    b.Property<float?>("MoMin");

                    b.Property<float?>("NiMax");

                    b.Property<float?>("NiMin");

                    b.Property<float?>("PMax");

                    b.Property<float?>("PMin");

                    b.Property<float?>("SMax");

                    b.Property<float?>("SMin");

                    b.Property<float?>("SiMax");

                    b.Property<float?>("SiMin");

                    b.Property<float?>("SnMax");

                    b.Property<float?>("SnMin");

                    b.Property<float?>("TeMax");

                    b.Property<float?>("TeMin");

                    b.Property<float?>("TiMax");

                    b.Property<float?>("TiMin");

                    b.Property<float?>("VMax");

                    b.Property<float?>("VMin");

                    b.HasKey("Code");

                    b.ToTable("ChemicalCompositionsInFurnaces");
                });

            modelBuilder.Entity("MNG.Models.ChemicalCompositionInLadle", b =>
                {
                    b.Property<string>("Code")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20);

                    b.Property<float?>("AlMax");

                    b.Property<float?>("AlMin");

                    b.Property<float?>("CCEMax");

                    b.Property<float?>("CCEMin");

                    b.Property<float?>("CMax");

                    b.Property<float?>("CMin");

                    b.Property<float?>("CrMax");

                    b.Property<float?>("CrMin");

                    b.Property<float?>("CuMax");

                    b.Property<float?>("CuMin");

                    b.Property<string>("Description");

                    b.Property<float?>("MgMax");

                    b.Property<float?>("MgMin");

                    b.Property<float?>("MnMax");

                    b.Property<float?>("MnMin");

                    b.Property<float?>("MoMax");

                    b.Property<float?>("MoMin");

                    b.Property<float?>("NiMax");

                    b.Property<float?>("NiMin");

                    b.Property<float?>("PMax");

                    b.Property<float?>("PMin");

                    b.Property<float?>("SMax");

                    b.Property<float?>("SMin");

                    b.Property<float?>("SiMax");

                    b.Property<float?>("SiMin");

                    b.Property<float?>("SnMax");

                    b.Property<float?>("SnMin");

                    b.Property<float?>("TeMax");

                    b.Property<float?>("TeMin");

                    b.Property<float?>("TiMax");

                    b.Property<float?>("TiMin");

                    b.Property<float?>("VMax");

                    b.Property<float?>("VMin");

                    b.HasKey("Code");

                    b.ToTable("ChemicalCompositionsInLadles");
                });

            modelBuilder.Entity("MNG.Models.ControlPlan", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("ChemicalCompositionInFurnaceCode")
                        .HasMaxLength(20);

                    b.Property<string>("ChemicalCompositionInLadleCode")
                        .HasMaxLength(20);

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(20);

                    b.Property<DateTime>("CreatedDate");

                    b.Property<string>("Description")
                        .HasMaxLength(1024);

                    b.Property<string>("MaterialSpecificationCode")
                        .HasMaxLength(20);

                    b.Property<string>("MeltingCode")
                        .HasMaxLength(20);

                    b.Property<string>("MoldingCode")
                        .HasMaxLength(20);

                    b.Property<string>("PouringCode")
                        .HasMaxLength(20);

                    b.Property<int>("ProductId");

                    b.Property<string>("Revision")
                        .HasMaxLength(50);

                    b.Property<string>("ShotBlastingCode")
                        .HasMaxLength(20);

                    b.Property<string>("ToolingCode")
                        .HasMaxLength(20);

                    b.Property<string>("Type");

                    b.HasKey("Id");

                    b.HasIndex("ChemicalCompositionInFurnaceCode");

                    b.HasIndex("ChemicalCompositionInLadleCode");

                    b.HasIndex("MaterialSpecificationCode");

                    b.HasIndex("MeltingCode");

                    b.HasIndex("MoldingCode");

                    b.HasIndex("PouringCode");

                    b.HasIndex("ProductId");

                    b.HasIndex("ShotBlastingCode");

                    b.HasIndex("ToolingCode");

                    b.ToTable("ControlPlans");
                });

            modelBuilder.Entity("MNG.Models.Customer", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Address");

                    b.Property<string>("CustomerCode")
                        .IsRequired()
                        .HasMaxLength(50);

                    b.Property<string>("CustomerCodeAc")
                        .HasMaxLength(50);

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50);

                    b.HasKey("Id");

                    b.ToTable("Customers");
                });

            modelBuilder.Entity("MNG.Models.DimensionSpeicification", b =>
                {
                    b.Property<string>("Code")
                        .HasMaxLength(20);

                    b.Property<string>("Description")
                        .HasMaxLength(1024);

                    b.Property<string>("MeasurementType");

                    b.Property<float>("NegativeTolerance");

                    b.Property<float>("Nominal");

                    b.Property<float>("PositiveTolerance");

                    b.HasKey("Code");

                    b.ToTable("DimensionSpecifications");
                });

            modelBuilder.Entity("MNG.Models.Material", b =>
                {
                    b.Property<string>("Code")
                        .HasMaxLength(20);

                    b.Property<string>("Description")
                        .HasMaxLength(1024);

                    b.HasKey("Code");

                    b.ToTable("Materials");
                });

            modelBuilder.Entity("MNG.Models.MaterialSpecification", b =>
                {
                    b.Property<string>("Code")
                        .HasMaxLength(20);

                    b.Property<float?>("CementiteMax");

                    b.Property<float?>("CementiteMin");

                    b.Property<string>("Description")
                        .HasMaxLength(1024);

                    b.Property<float?>("ElongationMax");

                    b.Property<float?>("ElongationMin");

                    b.Property<float?>("FerriteMax");

                    b.Property<float?>("FerriteMin");

                    b.Property<float?>("GraphiteAMax");

                    b.Property<float?>("GraphiteAMin");

                    b.Property<int?>("HBMax");

                    b.Property<int?>("HBMin");

                    b.Property<float?>("NodularityMax");

                    b.Property<float?>("NodularityMin");

                    b.Property<float?>("NoduleCount");

                    b.Property<float?>("PearliteMax");

                    b.Property<float?>("PearliteMin");

                    b.Property<float?>("SizeMax");

                    b.Property<float?>("SizeMin");

                    b.Property<float?>("SizeTensileMax");

                    b.Property<float?>("SizeTensileMin");

                    b.Property<int?>("TensileMax");

                    b.Property<int?>("TensileMin");

                    b.Property<float?>("YieldMax");

                    b.Property<float?>("YieldMin");

                    b.HasKey("Code");

                    b.ToTable("MaterialSpecifications");
                });

            modelBuilder.Entity("MNG.Models.MeltStandard", b =>
                {
                    b.Property<string>("Code")
                        .HasMaxLength(20);

                    b.Property<float?>("C_FC");

                    b.Property<float?>("C_FCD");

                    b.Property<string>("Description")
                        .HasMaxLength(1024);

                    b.Property<float?>("Fe_Mn");

                    b.Property<float?>("Fe_Mo");

                    b.Property<float?>("Fe_Ni");

                    b.Property<float?>("Fe_Si");

                    b.Property<float?>("HC_Cr");

                    b.Property<float?>("PigFC");

                    b.Property<float?>("PigFCD");

                    b.Property<float?>("RS");

                    b.Property<float?>("SS");

                    b.HasKey("Code");

                    b.ToTable("MeltStandards");
                });

            modelBuilder.Entity("MNG.Models.MoldStandard", b =>
                {
                    b.Property<string>("Code")
                        .HasMaxLength(20);

                    b.Property<int>("CSE");

                    b.Property<int>("CompressiveMax");

                    b.Property<int>("CompressiveMin");

                    b.Property<float>("CoreSettingTime");

                    b.Property<string>("Description")
                        .HasMaxLength(1024);

                    b.Property<float>("ExtendedSqueezeTime");

                    b.Property<int>("MoldHBMin");

                    b.Property<float>("MoldPositionCorr");

                    b.Property<int>("MoldSizeMax");

                    b.Property<int>("MoldSizeMin");

                    b.Property<float>("MoldThicknessCorr");

                    b.Property<float>("PPStrippingAcc");

                    b.Property<float>("PPStrippingDist");

                    b.Property<float>("SPStrippingAcc");

                    b.Property<float>("SPStrippingDist");

                    b.Property<float>("ShotPressure");

                    b.Property<float>("ShotTimeCorr");

                    b.Property<float>("SqueezePressure");

                    b.HasKey("Code");

                    b.ToTable("MoldStandards");
                });

            modelBuilder.Entity("MNG.Models.PourStandard", b =>
                {
                    b.Property<string>("Code")
                        .HasMaxLength(20);

                    b.Property<float?>("ChillMax");

                    b.Property<float?>("Cu");

                    b.Property<float?>("CuTol");

                    b.Property<string>("Description")
                        .HasMaxLength(1024);

                    b.Property<float?>("FeddTempTol");

                    b.Property<float?>("FeedTemp");

                    b.Property<float?>("Inoculant");

                    b.Property<float?>("InoculantTol");

                    b.Property<float?>("Magnesium");

                    b.Property<float?>("MagnesiumTol");

                    b.Property<float?>("Sn");

                    b.Property<float?>("SnTol");

                    b.Property<int>("TappingWg");

                    b.Property<float?>("WiredInoc");

                    b.Property<float?>("WiredInocTol");

                    b.Property<float?>("WiredMg");

                    b.Property<float?>("WiredMgTol");

                    b.HasKey("Code");

                    b.ToTable("PourStandards");
                });

            modelBuilder.Entity("MNG.Models.Product", b =>
                {
                    b.Property<int>("Id");

                    b.Property<int?>("ActiveControlPlanId");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(50);

                    b.Property<int>("CustomerId");

                    b.Property<string>("CustomerPartNo")
                        .HasMaxLength(100);

                    b.Property<string>("DimensionCode");

                    b.Property<string>("Drawing");

                    b.Property<byte[]>("Image");

                    b.Property<string>("MaterialCode")
                        .IsRequired();

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50);

                    b.Property<float>("Weight");

                    b.HasKey("Id");

                    b.HasIndex("ActiveControlPlanId");

                    b.HasIndex("CustomerId");

                    b.HasIndex("DimensionCode");

                    b.HasIndex("MaterialCode");

                    b.ToTable("Products");
                });

            modelBuilder.Entity("MNG.Models.Productions.Charging", b =>
                {
                    b.Property<string>("ChargeNo")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20);

                    b.Property<float?>("C_FC");

                    b.Property<float?>("C_FCD");

                    b.Property<DateTime>("ChargeTime");

                    b.Property<int?>("ControlPlanId");

                    b.Property<float?>("Fe_Mn");

                    b.Property<float?>("Fe_Mo");

                    b.Property<float?>("Fe_Ni");

                    b.Property<float?>("Fe_Si");

                    b.Property<float?>("HC_Cr");

                    b.Property<TimeSpan>("Interval");

                    b.Property<bool>("IsCompleted");

                    b.Property<string>("LotNoCode")
                        .IsRequired();

                    b.Property<int?>("MaxTemp");

                    b.Property<double>("MaxTempKwHr");

                    b.Property<DateTime>("MaxTempTime");

                    b.Property<double?>("MoltenMetal");

                    b.Property<float?>("PigFC");

                    b.Property<float?>("PigFCD");

                    b.Property<double>("PowerComp");

                    b.Property<int?>("ProductId");

                    b.Property<float?>("RS");

                    b.Property<double?>("RemainedMetal");

                    b.Property<float?>("SS");

                    b.Property<double>("StartKwHr");

                    b.Property<string>("Status");

                    b.Property<double?>("Total");

                    b.HasKey("ChargeNo");

                    b.HasIndex("ControlPlanId");

                    b.HasIndex("LotNoCode");

                    b.HasIndex("ProductId");

                    b.ToTable("Chargings");
                });

            modelBuilder.Entity("MNG.Models.Productions.Furnace", b =>
                {
                    b.Property<string>("Code")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20);

                    b.Property<string>("Brand");

                    b.Property<int>("Capacity");

                    b.Property<string>("Name");

                    b.Property<int>("Power");

                    b.HasKey("Code");

                    b.ToTable("Furnaces");
                });

            modelBuilder.Entity("MNG.Models.Productions.Kanban", b =>
                {
                    b.Property<string>("Code")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20);

                    b.Property<float?>("Chill");

                    b.Property<int?>("ControlPlanId");

                    b.Property<float?>("Copper");

                    b.Property<float?>("FeedTemp");

                    b.Property<float?>("Inoculant");

                    b.Property<bool>("IsCompleted");

                    b.Property<bool>("IsPassed");

                    b.Property<float>("KwHr");

                    b.Property<float?>("Magnesium");

                    b.Property<string>("MaterialCode");

                    b.Property<int?>("TappingTemp");

                    b.Property<string>("TestChemicalCompositionCode")
                        .IsRequired()
                        .HasMaxLength(20);

                    b.Property<DateTime>("Time");

                    b.Property<float?>("Tin");

                    b.Property<DateTime>("UpdatedTime");

                    b.Property<float>("Weight");

                    b.Property<float?>("WireInoculant");

                    b.Property<float?>("WireMagnesium");

                    b.HasKey("Code");

                    b.HasIndex("ControlPlanId");

                    b.HasIndex("MaterialCode");

                    b.HasIndex("TestChemicalCompositionCode");

                    b.ToTable("Kanbans");
                });

            modelBuilder.Entity("MNG.Models.Productions.Line", b =>
                {
                    b.Property<string>("Code")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20);

                    b.Property<string>("Name");

                    b.Property<string>("Size");

                    b.HasKey("Code");

                    b.ToTable("Line");
                });

            modelBuilder.Entity("MNG.Models.Productions.LotNo", b =>
                {
                    b.Property<string>("Code")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20);

                    b.Property<DateTime>("Date");

                    b.Property<string>("FurnaceCode");

                    b.Property<bool>("IsCompleted");

                    b.Property<string>("Shift")
                        .HasMaxLength(1);

                    b.HasKey("Code");

                    b.HasIndex("FurnaceCode");

                    b.ToTable("LotNos");
                });

            modelBuilder.Entity("MNG.Models.Productions.Molding", b =>
                {
                    b.Property<string>("Code")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20);

                    b.Property<float?>("Compressibility");

                    b.Property<string>("LineCode");

                    b.Property<int>("NoOfMold");

                    b.Property<DateTime>("Time");

                    b.HasKey("Code");

                    b.HasIndex("LineCode");

                    b.ToTable("Molding");
                });

            modelBuilder.Entity("MNG.Models.Productions.Notification", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Application");

                    b.Property<string>("Code");

                    b.Property<string>("Source");

                    b.Property<DateTime>("Time");

                    b.Property<string>("Type");

                    b.HasKey("Id");

                    b.ToTable("Notifications");
                });

            modelBuilder.Entity("MNG.Models.Productions.Plan", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Creator");

                    b.Property<DateTime>("DateCreated");

                    b.Property<int>("Factory");

                    b.Property<string>("Shift")
                        .HasMaxLength(1);

                    b.HasKey("Id");

                    b.ToTable("Plans");
                });

            modelBuilder.Entity("MNG.Models.Productions.PlanDetail", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("NoOfLadle");

                    b.Property<int>("PlanId");

                    b.Property<int>("ProductId");

                    b.Property<string>("Status");

                    b.HasKey("Id");

                    b.HasIndex("PlanId");

                    b.HasIndex("ProductId");

                    b.ToTable("PlanDetails");
                });

            modelBuilder.Entity("MNG.Models.Productions.Pouring", b =>
                {
                    b.Property<string>("Code")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20);

                    b.Property<TimeSpan?>("Duration");

                    b.Property<DateTime?>("FirstMoldTime");

                    b.Property<int?>("FirstTemp");

                    b.Property<bool?>("IsCompleted");

                    b.Property<bool?>("IsDurationOK");

                    b.Property<bool?>("IsFirstTempOK");

                    b.Property<bool?>("IsLastTempOK");

                    b.Property<bool?>("IsNoOfMoldOK");

                    b.Property<bool?>("IsPassed");

                    b.Property<string>("KanbanCode")
                        .IsRequired();

                    b.Property<DateTime?>("LastMoldTime");

                    b.Property<int?>("LastTemp");

                    b.Property<string>("LineCode")
                        .IsRequired();

                    b.Property<string>("MoldingCode");

                    b.Property<int?>("NoOfPouredMold");

                    b.Property<decimal>("PouredWeight")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<int>("ProductCode");

                    b.Property<int>("ProductNo");

                    b.HasKey("Code");

                    b.HasIndex("KanbanCode");

                    b.HasIndex("LineCode");

                    b.HasIndex("MoldingCode");

                    b.HasIndex("ProductCode");

                    b.ToTable("Pourings");
                });

            modelBuilder.Entity("MNG.Models.Productions.RawMaterial", b =>
                {
                    b.Property<string>("MatCode")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20);

                    b.Property<double?>("C_FC");

                    b.Property<double?>("C_FCD");

                    b.Property<double?>("Fe_Mn");

                    b.Property<double?>("Fe_Mo");

                    b.Property<double?>("Fe_Ni");

                    b.Property<double?>("Fe_Si");

                    b.Property<double?>("HC_Cr");

                    b.Property<double?>("PigFC");

                    b.Property<double?>("PigFCD");

                    b.Property<double?>("RS");

                    b.Property<double?>("RemainedMetal");

                    b.Property<double?>("SS");

                    b.Property<double?>("Total");

                    b.HasKey("MatCode");

                    b.ToTable("RawMaterials");
                });

            modelBuilder.Entity("MNG.Models.Productions.TestChemicalComposition", b =>
                {
                    b.Property<string>("Code")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20);

                    b.Property<string>("ChargingCode")
                        .IsRequired();

                    b.Property<int?>("ControlPlanId");

                    b.Property<bool>("IsCompleted");

                    b.Property<bool>("IsPassed");

                    b.Property<int?>("ProductId");

                    b.Property<DateTime>("Time");

                    b.Property<DateTime>("UpdatedTime");

                    b.HasKey("Code");

                    b.HasIndex("ChargingCode");

                    b.HasIndex("ControlPlanId");

                    b.HasIndex("ProductId");

                    b.ToTable("TestChemicalCompositions");
                });

            modelBuilder.Entity("MNG.Models.Productions.TestLog", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("TestCode");

                    b.Property<DateTime>("Time");

                    b.Property<string>("Type");

                    b.HasKey("Id");

                    b.ToTable("TestLogs");
                });

            modelBuilder.Entity("MNG.Models.Setting", b =>
                {
                    b.Property<string>("Code")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20);

                    b.Property<string>("API_URL");

                    b.Property<bool>("IsSelected");

                    b.Property<string>("Path");

                    b.Property<string>("PowerMeterIP");

                    b.Property<int>("RefreshRate");

                    b.Property<string>("Result_Path");

                    b.HasKey("Code");

                    b.ToTable("Settings");
                });

            modelBuilder.Entity("MNG.Models.ShotBlastStandard", b =>
                {
                    b.Property<string>("Code")
                        .HasMaxLength(20);

                    b.Property<float>("BallSize");

                    b.Property<int>("Current");

                    b.Property<string>("Description")
                        .HasMaxLength(1024);

                    b.Property<int>("Duration");

                    b.Property<int?>("ShakeOutTemp");

                    b.Property<int?>("UltraSonic");

                    b.HasKey("Code");

                    b.ToTable("ShotBlastStandards");
                });

            modelBuilder.Entity("MNG.Models.Tooling", b =>
                {
                    b.Property<string>("Code")
                        .HasMaxLength(20);

                    b.Property<string>("CBId");

                    b.Property<byte[]>("CBImage");

                    b.Property<string>("CSEId");

                    b.Property<byte[]>("CSEImage");

                    b.Property<int?>("Cavity");

                    b.Property<DateTime>("Date");

                    b.Property<string>("Description")
                        .HasMaxLength(1024);

                    b.Property<int>("FirstTemp");

                    b.Property<int>("LastTemp");

                    b.Property<float?>("MinDistance");

                    b.Property<int>("NoOfMoldPerLadle");

                    b.Property<float?>("PPHeight");

                    b.Property<string>("PPId");

                    b.Property<byte[]>("PPImage");

                    b.Property<float?>("PPThickness");

                    b.Property<int>("PouringTime");

                    b.Property<float?>("SPHeight");

                    b.Property<string>("SPId");

                    b.Property<byte[]>("SPImage");

                    b.Property<float?>("SPThickness");

                    b.Property<double>("TotalWeight");

                    b.HasKey("Code");

                    b.ToTable("Toolings");
                });

            modelBuilder.Entity("MNG.Models.User", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("Department");

                    b.Property<bool>("Inactive");

                    b.Property<string>("Name")
                        .IsRequired();

                    b.Property<string>("Password")
                        .IsRequired();

                    b.Property<int>("Position");

                    b.Property<DateTime>("Register_Date");

                    b.HasKey("Id");

                    b.ToTable("Users");
                });

            modelBuilder.Entity("MNG.Models.ControlPlan", b =>
                {
                    b.HasOne("MNG.Models.ChemicalCompositionInFurnace", "ChemicalCompositionInFurnace")
                        .WithMany("ControlPlans")
                        .HasForeignKey("ChemicalCompositionInFurnaceCode");

                    b.HasOne("MNG.Models.ChemicalCompositionInLadle", "ChemicalCompositionInLadle")
                        .WithMany("ControlPlans")
                        .HasForeignKey("ChemicalCompositionInLadleCode");

                    b.HasOne("MNG.Models.MaterialSpecification", "MaterialSpecification")
                        .WithMany("ControlPlans")
                        .HasForeignKey("MaterialSpecificationCode");

                    b.HasOne("MNG.Models.MeltStandard", "Melting")
                        .WithMany("ControlPlans")
                        .HasForeignKey("MeltingCode");

                    b.HasOne("MNG.Models.MoldStandard", "Molding")
                        .WithMany("ControlPlans")
                        .HasForeignKey("MoldingCode");

                    b.HasOne("MNG.Models.PourStandard", "Pouring")
                        .WithMany("ControlPlans")
                        .HasForeignKey("PouringCode");

                    b.HasOne("MNG.Models.Product", "Product")
                        .WithMany("ControlPlans")
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("MNG.Models.ShotBlastStandard", "ShotBlasting")
                        .WithMany("ControlPlans")
                        .HasForeignKey("ShotBlastingCode");

                    b.HasOne("MNG.Models.Tooling", "Tooling")
                        .WithMany("ControlPlans")
                        .HasForeignKey("ToolingCode");
                });

            modelBuilder.Entity("MNG.Models.Product", b =>
                {
                    b.HasOne("MNG.Models.ControlPlan", "ActiveControlPlan")
                        .WithMany()
                        .HasForeignKey("ActiveControlPlanId");

                    b.HasOne("MNG.Models.Customer", "Customer")
                        .WithMany()
                        .HasForeignKey("CustomerId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("MNG.Models.DimensionSpeicification", "Dimension")
                        .WithMany()
                        .HasForeignKey("DimensionCode");

                    b.HasOne("MNG.Models.Material", "Material")
                        .WithMany()
                        .HasForeignKey("MaterialCode")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("MNG.Models.Productions.Charging", b =>
                {
                    b.HasOne("MNG.Models.ControlPlan", "ControlPlan")
                        .WithMany()
                        .HasForeignKey("ControlPlanId");

                    b.HasOne("MNG.Models.Productions.LotNo", "LotNo")
                        .WithMany("Chargings")
                        .HasForeignKey("LotNoCode")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("MNG.Models.Product", "Product")
                        .WithMany()
                        .HasForeignKey("ProductId");
                });

            modelBuilder.Entity("MNG.Models.Productions.Kanban", b =>
                {
                    b.HasOne("MNG.Models.ControlPlan", "ControlPlan")
                        .WithMany()
                        .HasForeignKey("ControlPlanId");

                    b.HasOne("MNG.Models.Material", "Material")
                        .WithMany()
                        .HasForeignKey("MaterialCode");

                    b.HasOne("MNG.Models.Productions.TestChemicalComposition", "TestChemicalComposition")
                        .WithMany("Kanbans")
                        .HasForeignKey("TestChemicalCompositionCode")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.OwnsOne("MNG.Models.Productions.MaterialQuantityValidation", "MatValidation", b1 =>
                        {
                            b1.Property<string>("KanbanCode");

                            b1.Property<bool?>("IsOk_Chill");

                            b1.Property<bool?>("IsOk_Copper");

                            b1.Property<bool?>("IsOk_FeedTemp");

                            b1.Property<bool?>("IsOk_Inoculant");

                            b1.Property<bool?>("IsOk_Magnesium");

                            b1.Property<bool?>("IsOk_Tin");

                            b1.Property<bool?>("IsOk_WireInoculant");

                            b1.Property<bool?>("IsOk_WireMagnesium");

                            b1.ToTable("Kanbans");

                            b1.HasOne("MNG.Models.Productions.Kanban")
                                .WithOne("MatValidation")
                                .HasForeignKey("MNG.Models.Productions.MaterialQuantityValidation", "KanbanCode")
                                .OnDelete(DeleteBehavior.Cascade);
                        });

                    b.OwnsOne("MNG.Models.Productions.ChemicalComposition", "Result", b1 =>
                        {
                            b1.Property<string>("KanbanCode");

                            b1.Property<float?>("Al");

                            b1.Property<float?>("C");

                            b1.Property<float?>("CCE");

                            b1.Property<float?>("Cr");

                            b1.Property<float?>("Cu");

                            b1.Property<float?>("Mg");

                            b1.Property<float?>("Mn");

                            b1.Property<float?>("Mo");

                            b1.Property<float?>("Ni");

                            b1.Property<float?>("P");

                            b1.Property<float?>("S");

                            b1.Property<float?>("Si");

                            b1.Property<float?>("Sn");

                            b1.Property<float?>("Te");

                            b1.Property<float?>("Ti");

                            b1.Property<float?>("V");

                            b1.ToTable("Kanbans");

                            b1.HasOne("MNG.Models.Productions.Kanban")
                                .WithOne("Result")
                                .HasForeignKey("MNG.Models.Productions.ChemicalComposition", "KanbanCode")
                                .OnDelete(DeleteBehavior.Cascade);
                        });

                    b.OwnsOne("MNG.Models.Productions.ChemicalCompositionValidation", "Validation", b1 =>
                        {
                            b1.Property<string>("KanbanCode");

                            b1.Property<bool?>("IsOk_Al");

                            b1.Property<bool?>("IsOk_C");

                            b1.Property<bool?>("IsOk_CCE");

                            b1.Property<bool?>("IsOk_Cr");

                            b1.Property<bool?>("IsOk_Cu");

                            b1.Property<bool?>("IsOk_Mg");

                            b1.Property<bool?>("IsOk_Mn");

                            b1.Property<bool?>("IsOk_Mo");

                            b1.Property<bool?>("IsOk_Ni");

                            b1.Property<bool?>("IsOk_P");

                            b1.Property<bool?>("IsOk_S");

                            b1.Property<bool?>("IsOk_Si");

                            b1.Property<bool?>("IsOk_Sn");

                            b1.Property<bool?>("IsOk_Te");

                            b1.Property<bool?>("IsOk_Ti");

                            b1.Property<bool?>("IsOk_V");

                            b1.ToTable("Kanbans");

                            b1.HasOne("MNG.Models.Productions.Kanban")
                                .WithOne("Validation")
                                .HasForeignKey("MNG.Models.Productions.ChemicalCompositionValidation", "KanbanCode")
                                .OnDelete(DeleteBehavior.Cascade);
                        });
                });

            modelBuilder.Entity("MNG.Models.Productions.LotNo", b =>
                {
                    b.HasOne("MNG.Models.Productions.Furnace", "Furnace")
                        .WithMany("LotNos")
                        .HasForeignKey("FurnaceCode");
                });

            modelBuilder.Entity("MNG.Models.Productions.Molding", b =>
                {
                    b.HasOne("MNG.Models.Productions.Line", "Line")
                        .WithMany()
                        .HasForeignKey("LineCode");
                });

            modelBuilder.Entity("MNG.Models.Productions.PlanDetail", b =>
                {
                    b.HasOne("MNG.Models.Productions.Plan", "Plan")
                        .WithMany("PlanDetails")
                        .HasForeignKey("PlanId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("MNG.Models.Product", "Product")
                        .WithMany("PlanDetails")
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("MNG.Models.Productions.Pouring", b =>
                {
                    b.HasOne("MNG.Models.Productions.Kanban", "Kanban")
                        .WithMany("Kanbans")
                        .HasForeignKey("KanbanCode")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("MNG.Models.Productions.Line", "Line")
                        .WithMany("Pourings")
                        .HasForeignKey("LineCode")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("MNG.Models.Productions.Molding", "Molding")
                        .WithMany("Kanbans")
                        .HasForeignKey("MoldingCode");

                    b.HasOne("MNG.Models.Product", "Product")
                        .WithMany()
                        .HasForeignKey("ProductCode")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.OwnsOne("MNG.Models.Productions.Defects.DefectCause", "Defect", b1 =>
                        {
                            b1.Property<string>("PouringCode");

                            b1.Property<double>("DefectRate");

                            b1.Property<double>("DefectWeightRate");

                            b1.Property<int>("TotalDefect");

                            b1.Property<double>("TotalDefectWeight");

                            b1.Property<int>("TotalGood");

                            b1.Property<double>("TotalGoodWeight");

                            b1.Property<int>("TotalNumber");

                            b1.Property<double>("TotalWeight");

                            b1.ToTable("Pourings");

                            b1.HasOne("MNG.Models.Productions.Pouring")
                                .WithOne("Defect")
                                .HasForeignKey("MNG.Models.Productions.Defects.DefectCause", "PouringCode")
                                .OnDelete(DeleteBehavior.Cascade);

                            b1.OwnsOne("MNG.Models.Productions.Defects.CoreDefect", "CoreDefect", b2 =>
                                {
                                    b2.Property<string>("DefectCausePouringCode");

                                    b2.Property<int>("CoreBroken");

                                    b2.Property<int>("CoreDeform");

                                    b2.Property<int>("Total");

                                    b2.Property<int>("WetCore");

                                    b2.ToTable("Pourings");

                                    b2.HasOne("MNG.Models.Productions.Defects.DefectCause")
                                        .WithOne("CoreDefect")
                                        .HasForeignKey("MNG.Models.Productions.Defects.CoreDefect", "DefectCausePouringCode")
                                        .OnDelete(DeleteBehavior.Cascade);
                                });

                            b1.OwnsOne("MNG.Models.Productions.Defects.EngineeringDefect", "EngDefect", b2 =>
                                {
                                    b2.Property<string>("DefectCausePouringCode");

                                    b2.Property<int>("AirPocket");

                                    b2.Property<int>("MissedIdentifer");

                                    b2.Property<int>("Total");

                                    b2.Property<int>("WornTooling");

                                    b2.ToTable("Pourings");

                                    b2.HasOne("MNG.Models.Productions.Defects.DefectCause")
                                        .WithOne("EngDefect")
                                        .HasForeignKey("MNG.Models.Productions.Defects.EngineeringDefect", "DefectCausePouringCode")
                                        .OnDelete(DeleteBehavior.Cascade);
                                });

                            b1.OwnsOne("MNG.Models.Productions.Defects.FinishingDefect", "FinDefect", b2 =>
                                {
                                    b2.Property<string>("DefectCausePouringCode");

                                    b2.Property<int>("Deform");

                                    b2.Property<int>("Dent");

                                    b2.Property<int>("OverGrinding");

                                    b2.Property<int>("Total");

                                    b2.ToTable("Pourings");

                                    b2.HasOne("MNG.Models.Productions.Defects.DefectCause")
                                        .WithOne("FinDefect")
                                        .HasForeignKey("MNG.Models.Productions.Defects.FinishingDefect", "DefectCausePouringCode")
                                        .OnDelete(DeleteBehavior.Cascade);
                                });

                            b1.OwnsOne("MNG.Models.Productions.Defects.MeltingDefect", "MeltDefect", b2 =>
                                {
                                    b2.Property<string>("DefectCausePouringCode");

                                    b2.Property<int>("BlowHole");

                                    b2.Property<int>("ChemNG");

                                    b2.Property<int>("Chill");

                                    b2.Property<int>("HardnessNG");

                                    b2.Property<int>("MicroNG");

                                    b2.Property<int>("PinHole");

                                    b2.Property<int>("Shrinkage");

                                    b2.Property<int>("Total");

                                    b2.ToTable("Pourings");

                                    b2.HasOne("MNG.Models.Productions.Defects.DefectCause")
                                        .WithOne("MeltDefect")
                                        .HasForeignKey("MNG.Models.Productions.Defects.MeltingDefect", "DefectCausePouringCode")
                                        .OnDelete(DeleteBehavior.Cascade);
                                });

                            b1.OwnsOne("MNG.Models.Productions.Defects.MoldingDefect", "MoldDefect", b2 =>
                                {
                                    b2.Property<string>("DefectCausePouringCode");

                                    b2.Property<int>("BlowHole");

                                    b2.Property<int>("Burr");

                                    b2.Property<int>("ChillMismatch");

                                    b2.Property<int>("CoreMismatch");

                                    b2.Property<int>("MissMatch");

                                    b2.Property<int>("PinHole");

                                    b2.Property<int>("SandBroken");

                                    b2.Property<int>("SandDrop");

                                    b2.Property<int>("Total");

                                    b2.ToTable("Pourings");

                                    b2.HasOne("MNG.Models.Productions.Defects.DefectCause")
                                        .WithOne("MoldDefect")
                                        .HasForeignKey("MNG.Models.Productions.Defects.MoldingDefect", "DefectCausePouringCode")
                                        .OnDelete(DeleteBehavior.Cascade);
                                });

                            b1.OwnsOne("MNG.Models.Productions.Defects.PouringDefect", "PourDefect", b2 =>
                                {
                                    b2.Property<string>("DefectCausePouringCode");

                                    b2.Property<int>("BlowHole");

                                    b2.Property<int>("Chill");

                                    b2.Property<int>("ColdShut");

                                    b2.Property<int>("HardnessNG");

                                    b2.Property<int>("MicroNG");

                                    b2.Property<int>("PinHole");

                                    b2.Property<int>("Shrinkage");

                                    b2.Property<int>("Slag");

                                    b2.Property<int>("Total");

                                    b2.ToTable("Pourings");

                                    b2.HasOne("MNG.Models.Productions.Defects.DefectCause")
                                        .WithOne("PourDefect")
                                        .HasForeignKey("MNG.Models.Productions.Defects.PouringDefect", "DefectCausePouringCode")
                                        .OnDelete(DeleteBehavior.Cascade);
                                });
                        });

                    b.OwnsOne("MNG.Models.Productions.QA.QAInspection", "QInspect", b1 =>
                        {
                            b1.Property<string>("PouringCode");

                            b1.Property<float>("Cementite");

                            b1.Property<int>("Count");

                            b1.Property<float>("Elongation");

                            b1.Property<float>("Ferrite");

                            b1.Property<float>("GraphiteA");

                            b1.Property<byte[]>("GraphiteImg");

                            b1.Property<int>("Hardness");

                            b1.Property<byte[]>("MatrixImg");

                            b1.Property<float>("Nodularity");

                            b1.Property<float>("Pearlite");

                            b1.Property<int>("Size");

                            b1.Property<float>("SizeTensile");

                            b1.Property<int>("Tensile");

                            b1.Property<int>("Yeild");

                            b1.ToTable("Pourings");

                            b1.HasOne("MNG.Models.Productions.Pouring")
                                .WithOne("QInspect")
                                .HasForeignKey("MNG.Models.Productions.QA.QAInspection", "PouringCode")
                                .OnDelete(DeleteBehavior.Cascade);
                        });
                });

            modelBuilder.Entity("MNG.Models.Productions.TestChemicalComposition", b =>
                {
                    b.HasOne("MNG.Models.Productions.Charging", "Charging")
                        .WithMany("TestChemicalCompositions")
                        .HasForeignKey("ChargingCode")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("MNG.Models.ControlPlan", "ControlPlan")
                        .WithMany()
                        .HasForeignKey("ControlPlanId");

                    b.HasOne("MNG.Models.Product", "Product")
                        .WithMany()
                        .HasForeignKey("ProductId");

                    b.OwnsOne("MNG.Models.Productions.ChemicalComposition", "Result", b1 =>
                        {
                            b1.Property<string>("TestChemicalCompositionCode");

                            b1.Property<float?>("Al");

                            b1.Property<float?>("C");

                            b1.Property<float?>("CCE");

                            b1.Property<float?>("Cr");

                            b1.Property<float?>("Cu");

                            b1.Property<float?>("Mg");

                            b1.Property<float?>("Mn");

                            b1.Property<float?>("Mo");

                            b1.Property<float?>("Ni");

                            b1.Property<float?>("P");

                            b1.Property<float?>("S");

                            b1.Property<float?>("Si");

                            b1.Property<float?>("Sn");

                            b1.Property<float?>("Te");

                            b1.Property<float?>("Ti");

                            b1.Property<float?>("V");

                            b1.ToTable("TestChemicalCompositions");

                            b1.HasOne("MNG.Models.Productions.TestChemicalComposition")
                                .WithOne("Result")
                                .HasForeignKey("MNG.Models.Productions.ChemicalComposition", "TestChemicalCompositionCode")
                                .OnDelete(DeleteBehavior.Cascade);
                        });

                    b.OwnsOne("MNG.Models.Productions.ChemicalCompositionValidation", "Validation", b1 =>
                        {
                            b1.Property<string>("TestChemicalCompositionCode");

                            b1.Property<bool?>("IsOk_Al");

                            b1.Property<bool?>("IsOk_C");

                            b1.Property<bool?>("IsOk_CCE");

                            b1.Property<bool?>("IsOk_Cr");

                            b1.Property<bool?>("IsOk_Cu");

                            b1.Property<bool?>("IsOk_Mg");

                            b1.Property<bool?>("IsOk_Mn");

                            b1.Property<bool?>("IsOk_Mo");

                            b1.Property<bool?>("IsOk_Ni");

                            b1.Property<bool?>("IsOk_P");

                            b1.Property<bool?>("IsOk_S");

                            b1.Property<bool?>("IsOk_Si");

                            b1.Property<bool?>("IsOk_Sn");

                            b1.Property<bool?>("IsOk_Te");

                            b1.Property<bool?>("IsOk_Ti");

                            b1.Property<bool?>("IsOk_V");

                            b1.ToTable("TestChemicalCompositions");

                            b1.HasOne("MNG.Models.Productions.TestChemicalComposition")
                                .WithOne("Validation")
                                .HasForeignKey("MNG.Models.Productions.ChemicalCompositionValidation", "TestChemicalCompositionCode")
                                .OnDelete(DeleteBehavior.Cascade);
                        });
                });
#pragma warning restore 612, 618
        }
    }
}
